    ...
    private void activateAssets(List<String> assetsLinks) {
        ...
        for (String assetUrl : assetsLinks) {
            try {
                ...
                boolean isInactive = asset != null && asset.has("status") && asset.get("status").asText().equals("inactive");
                ...
                if (isInactive) {
                    ...
                    String activateResponse = restTemplate.exchange(assetActivationLink, HttpMethod.POST, activationRequest, String.class).getBody();
                ...
                boolean isActivating = asset != null && asset.has("status") && asset.get("status").asText().equals("activating");

                if (isActivating) {
                    continue;
                }

                boolean isActive = asset != null && asset.has("status") && asset.get("status").asText().equals("active");
